...
function Board() {
  ...
  const [numTiles, setNumTiles] = useState(
    answers[CURRENT_ANSWER] ? answers[CURRENT_ANSWER].length : 0
  );
  ...
  function incrementGuessLetter() {
    setGuessLetter((prevState) => prevState + 1);
  }
  function decrementGuessLetter() {
    setGuessLetter((prevState) => prevState - 1);
  }
  function incrementGuessWordCount() {
    setGuessWordCount((prevState) => prevState + 1);
  }
  function stopGame() {
    setIsStopped(true);
  }
  function updateKeyboard(guessWord) {
    setKeyboardUpdate((prevState) => ({
  ...
  function updateTiles() {
    ...
    setTimeout(() => {
      setNumTiles(
        answers[CURRENT_ANSWER + 1]
          ? answers[CURRENT_ANSWER + 1].length
          : answers[CURRENT_ANSWER].length
      );
    }, DELAY);
  }
  ...
  const handleKeyDown = (event) => {
    ...
    // Deleting guessed letter
      ...
      event.key === "Backspace" &&
      guessLetter >= guessWordCount * numTiles
    ) {
      if (guessLetter !== guessWordCount * numTiles) {
        document.getElementById(ids[guessLetter - 1]).innerText = "";
        document.getElementById(ids[guessLetter - 1]).style.borderColor =
          "rgb(211, 214, 218)"; // light grey
    ...
    // Adding guessed letter
      ...
      ids.includes(event.key) &&
      guessLetter < (guessWordCount + 1) * numTiles
    ) {
      document.getElementById(ids[guessLetter]).innerText = event.key;
      document.getElementById(ids[guessLetter]).style.borderColor =
        "rgb(129, 131, 132)"; // dark grey
      incrementGuessLetter();
    }
    // Word is guessed, move to next row
    else if (
      event.key === "Enter" &&
      guessLetter % numTiles === 0 &&
      guessLetter !== 0
    ) {
      ...
      if (correctGuess()) {
        ...
        updateAnswer();
        updateTiles();
        if (removeEmpty(answers).length == 1) {
          // if current answer is last answer remaining
          stopGame();
        } else {
          restartBoard();
  ...
  return (
    <div>
      {[...Array(numRows)].map(() => (
        <div className="row">
          {[...Array(numTiles)].map(() => (
            <div className="board-tile" id={ids[idCount++]}></div>
          ))}
        </div>
      ))}
    </div>
